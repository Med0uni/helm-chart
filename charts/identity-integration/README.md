...
![Version: 0.1.8](https://img.shields.io/badge/Version-0.1.8-informational?style=flat-square) ![AppVersion: 2022.02](https://img.shields.io/badge/AppVersion-2022.02-informational?style=flat-square)

A Helm Chart for Backbase Identity Integration

## Requirements

| Repository | Name | Version |
|------------|------|---------|
| https://repo.backbase.com/backbase-charts | identityintegrationservice(backbase-app) | 0.23.8 |

## Values

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| global.activemq | object | `{"checkEnabled":true,"enabled":true}` | JMS/ActiveMQ settings |
| global.activemq.checkEnabled | bool | `true` | If true, will create an initContainer which waits for ActiveMQ to be ready |
| global.activemq.enabled | bool | `true` | If enabled, will set automatically the following environment variables: SPRING_ACTIVEMQ_BROKER_URL, SPRING_ACTIVEMQ_USER, SPRING_ACTIVEMQ_PASSWORD |
| global.app | object | `{"image":{"registry":"","repository":"identity-integration-service","tag":""},"metadata":{"tier":"identity"}}` | Configures metadata and image of the app to be deployed |
| global.app.image | object | `{"registry":"","repository":"identity-integration-service","tag":""}` | Configures the docker image to use |
| global.app.image.registry | string | `""` | The docker registry for this app |
| global.app.image.repository | string | `"identity-integration-service"` | Name of the Docker image repository |
| global.app.image.tag | string | `""` | The docker image tag for this app |
| global.app.metadata | object | `{"tier":"identity"}` | Configures additional `app.backbase.com/key: value` labels for deployments |
| global.app.metadata.tier | string | `"identity"` | The product the app belongs to |
| global.jwt | object | `{"SIG_SECRET_KEY_fromSecret":true,"USERCTX_KEY_fromSecret":true,"enabled":true,"existingSecret":"backbase-env-jwt"}` | Sets the JWT-related environment variables in the pod |
...
| global.service | object | `{"enabled":true}` | Kubernetes service |
| global.service.enabled | bool | `true` | If enabled, will create a Kubernetes service resource for this deployment |
| identityintegrationservice | object | `{"app":{"name":"identityintegrationservice","resources":{"memory":{"ram":512}}},"database":{"checkEnabled":false,"enabled":false},"enabled":true,"env":{"IDENTITY_SERVERURL":"http://identity-backbaseidentity:8080/auth"},"service":{"nameOverride":"identity-integration-service"}}` | Kubernetes service |
| identityintegrationservice.app | object | `{"name":"identityintegrationservice","resources":{"memory":{"ram":512}}}` | Application configuration settings |
| identityintegrationservice.app.name | string | `"identityintegrationservice"` | Name of the application |
| identityintegrationservice.app.resources | object | `{"memory":{"ram":512}}` | Resource configuration settings |
| identityintegrationservice.app.resources.memory | object | `{"ram":512}` | Memory configuration |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| global.activemq | object | `{"checkEnabled":true,"checkInitContainerCpu":"100m","checkInitContainerImage":"busybox","checkInitContainerMemory":"100Mi","checkType":"netcat","enabled":true,"existingSecret":"","existingSecretKey":"ACTIVEMQ_PASSWORD","host":"activemq","password":"CHANGEME","port":61616,"urlTemplate":"","username":"admin"}` | JMS/ActiveMQ settings |
| global.activemq.checkEnabled | bool | `true` | If true, will create an initContainer which waits for ActiveMQ to be ready |
...
| global.app | object | `{"image":{"pullPolicy":"IfNotPresent","registry":"","repository":"identity-integration-service","tag":""},"metadata":{"tier":"identity"}}` | Configures metadata and image of the app to be deployed |
| global.app.image | object | `{"pullPolicy":"IfNotPresent","registry":"","repository":"identity-integration-service","tag":""}` | Configures the docker image to use |
...
| identityintegrationservice.app.resources.cpu.maxShares | int | `1000` | (int) Amount of shares of a CPU unit to limit to (where 1000=1 CPU). |
...
| identityintegrationservice.persistence.enabled | bool | `false` | If true, will create and mount a '*-data' Volume in the deployment spec. The value of `persistence.existingClaim` determines if this is a new or pre-existing Persistent Volume Claim. |
